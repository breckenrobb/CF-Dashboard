...
ggplot(data, aes(x={{xvar}}, y={{yvar}}, group={{grp}}, colour = {{grp}})) +
  geom_line(linewidth = 4, stat = "identity",colour="black") + 
  geom_line(linewidth = 3, stat = "identity") +
...
spaghetti_plot_wateryr <- function(data, var, col,CF,CFmethod=""){
  df <- deparse(substitute(data))
    ggplot() +
      geom_line(data=data,aes(x=WaterYr,y=eval(parse(text=var)),group=Year),colour=col,linewidth=2) +
...
LT_plot <-function(data,yvar, rollvar,cols,yaxis,title,CFmethod=""){
  ggplot(data, aes(x=Year, y={{yvar}}, col=CF, fill=CF)) + 
   geom_rect(xmin=Yr-Range/2, xmax=Yr+Range/2, ymin=0, ymax=100, alpha=0.1, fill="lightgray", col="lightgray") +
    ...
    geom_line(linewidth=2) + geom_point(col="black", linewidth=4, shape=16) +
    geom_point() +
    geom_line(aes(x=Year, y={{rollvar}}),linewidth=3,colour="black", na.rm=TRUE) +
    geom_line(aes(x=Year, y={{rollvar}},colour = CF), linewidth=2 ,na.rm=TRUE) +
